...
float3 GetNormalFromHeightWS(float3 positionWS, float3 vertexNormalWS, float height)
{
    positionWS = positionWS + height * vertexNormalWS;
    float3 tangentWS = ddx(positionWS);
    float3 bitangentWS = ddy(positionWS);
...
float4 FastPow(float4 x, float n)
{
    n = n * 1.4427f + 1.4427f; // 1.4427f --> 1/ln(2)
    return exp2(x * n - n);
}
...
real2 LerpWhiteTo(real2 b, real t)
{
    real oneMinusT = 1.0 - t;
    return real2(oneMinusT, oneMinusT) + b * t;
}
...
real LerpToWhite(real a, real t)
{
    return (1.0 - t) * a + t;
}

real3 LerpToWhite(real3 a, real t)
{
    return real3(t, t, t) + (1.0 - t) * a;
}
