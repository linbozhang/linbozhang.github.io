...
real3 FastestLinearToSRGB(real3 c)
{
    return sqrt(c);
}

real3 FastestSRGBToLinear(real3 c)
{
    return c * c;
}
...
real3 RgbToHcv(real3 rgb)
{
    const real4 K = real4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
    real4 p = lerp(real4(rgb.bg, K.wz), real4(rgb.gb, K.xy), step(rgb.b, rgb.g));
    real4 q = lerp(real4(p.xyw, rgb.r), real4(rgb.r, p.yzx), step(p.x, rgb.r));
    real c = q.x - min(q.w, q.y);
    const real e = 1.0e-4;
    real h = abs(q.z + (q.w - q.y) / (6.0 * c + e));
    return real3(h, c, q.x);
}

real3 HueToRgb(real hue)
{
    return saturate(abs(hue * 6 - real3(3, 2, 4)) * real3(1, -1, -1) + real3(-1, 2, 2));
}
...
//  Sat [0.0, 1.0]
//  Lum [0.0, HALF_MAX]
real3 RgbToHsl(real3 rgb)
{
    real3 hcv = RgbToHcv(rgb);
    real l = hcv.z - hcv.y * 0.5f;
    real s = hcv.y / (1 - abs(l * 2 - 1) + 1e-04f);
    return real3(hcv.x, s, l);
}

real3 HslToRgb(real3 hsl)
{
    const real3 rgb = HueToRgb(hsl.x);
    const real c = (1 - abs(2 * hsl.z - 1)) * hsl.y;
...
#else
    return (pow(10.0, (x - LogC_D) / LogC_C) - LogC_B) / LogC_A;
#endif
}
...
// This will cause precision lost when value is high.
// Only suitable for color.
half3 EncodeHDRColor(half3 color, float kSqrtMaxValue)
{
    color = sqrt(max(0, color));
    return min(color / kSqrtMaxValue, 1);
}

half3 DecodeHDRColor(half3 color, float kSqrtMaxValue)
{
    color *= kSqrtMaxValue;
    return color * color;
}
